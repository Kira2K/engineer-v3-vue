    window.table_filter = filter => row =>
      window.fields.reduce((acc, field) => {
        if (field.type == 'number' || field.type == "date") return acc && (!filter[field.name]  || `${row[field.name]}`.startsWith(filter[field.name]))
        return acc && (!filter[field.name] || (row[field.name] || '').toLowerCase().includes(filter[field.name].toLowerCase()))
      }, true)
    ...
    ;$(() => {
        ...
        $('#cols_apply').click(() => {
          ...
          document.location.href = `/${window.table_url}?${urlSearchParams.toString()}`
        })
        $('#cols_reset').click(() => {
